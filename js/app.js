        ...
        function (window, noGlobal) {
          ...
          jQuery.extend({
            ...
            text: function (elem) {
              ...
              if (!nodeType)
                while ((node = elem[i++])) ret += jQuery.text(node);
  ...
  (() => {
    ...
    (function (exports) {
      ...
      function emit$1(event, ...args) {
        if (exports.devtools) exports.devtools.emit(event, ...args);
      ...
      const computed = (getterOrOptions, debugOptions) =>
        computed$1(getterOrOptions, debugOptions, isInSSRComponentSetup);
      ...
      const defineSSRCustomElement = (options) =>
        defineCustomElement(options, hydrate);
      const BaseClass =
        ...
        while (!isEnd(context, mode, ancestors)) {
          ...
          if (mode === 0 || mode === 1)
            if (!context.inVPre && startsWith(s, context.options.delimiters[0]))
              ...
              if (s.length === 1) emitError(context, 5, 1);
              else if (s[1] === "!")
                if (startsWith(s, "\x3c!--")) node = parseComment(context);
                ...
                if (s.length === 2) emitError(context, 5, 2);
                else if (s[2] === ">") {
      ...
      function parseCDATA(context, ancestors) {
        ...
        if (context.source.length === 0) emitError(context, 6);
      ...
      function parseTag(context, type, parent) {
        ...
        if (context.source.length === 0) emitError(context, 9);
        else {
      ...
      function parseTextData(context, length, mode) {
        ...
        if (mode === 2 || mode === 3 || !rawText.includes("&")) return rawText;
      ...
      const transformIf = createStructuralDirectiveTransform(
        /^(if|else|else-if)$/,
        (node, dir, context) =>
          processIf(node, dir, context, (ifNode, branch, isRoot) => {
      ...
      const parserOptions = {
        ...
        isBuiltInComponent: (tag) => {
          if (isBuiltInType(tag, `Transition`)) return TRANSITION;
      ...
      const transformOn = (dir, node, context) =>
        transformOn$1(dir, node, context, (baseResult) => {
          const { modifiers } = dir;
      ...
      if (spollersArray.length > 0) {
        ...
        if (spollersClose.length)
          document.addEventListener("click", function (e) {
            const el = e.target;
            if (
              !el.closest("[data-spollers]") ||
              el.closest(".sorting-box__close")
            )
    ...
    function process() {
      const items = document.querySelectorAll(".s-process__item");

      if (items.length) {
        let currentActive = 0;

        const buttons = document.querySelectorAll(".s-process__btn");
        const nextButtons = document.querySelectorAll(".s-process__item .next");

        nextButtons.forEach((btn) => {
          btn.addEventListener("click", () => changeActive(currentActive + 1));
        });
    ...
    function sliders() {
      ...
      if (resSlider) {
        const swiper = new Swiper(resSlider, {
          ...
          pagination: {
            el: ".s-res .slider-pagination",
            clickable: true,
          },
        });
      }

      const resRecSlider = document.querySelector(".s-res__rec-slider");

      if (resRecSlider) {
        const swiper = new Swiper(resRecSlider, {
          speed: 800,
          spaceBetween: 20,
          slidesPerView: 2,
          breakpoints: {
